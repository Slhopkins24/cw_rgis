...
mtcars_g35 <- select(df_mtcars, c(gear))
...
select(df_mtcars, c(wt))
mtcars_light_cyl4 <- df_mtcars %>% filter(wt<2.5 & cyl == 4)

# 5. Sort `df_mtcars` by horsepower (`hp`) in ascending order
# Use `arrange()` to sort the data.
# Assign to: `mtcars_hp`
select(df_mtcars, c(hp))
...
g_scat <- ggplot(data = iris,
       mapping = aes(x = Sepal.Length,
                     y = Sepal.Width)) + geom_point()
g_scat + labs(x = 'Sepal.Width', y = 'Sepal.Length')
...
g_scat_col <- iris %>% ggplot(aes(x = Sepal.Length,
                           y = Sepal.Width, color = Species)) + geom_point()
g_scat_col + labs(x = 'Petal.Width', y = 'Petal.Length')
...
g_hist <- iris %>%  ggplot(aes(x = Petal.Width)) + geom_histogram(binwidth = 0.5)
