...
# Reference: Chapter 2
sf_site <- st_as_sf(df_site, coords = c("lon", "lat"), crs = 4326)
...
# Reference: Chapter 3
sf_site_w_county <- st_join(sf_site, sf_nc_county)
...
ggplot() +
  ...
  geom_sf(data = sf_site,size = 1) +
  theme_minimal() +
  labs(title = "Fish Sampling Sites by County (NC)")
...
sf_county_proj <- st_transform(sf_county_s1, crs = 26917)
